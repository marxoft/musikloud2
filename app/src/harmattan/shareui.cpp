...
bool ShareUi::shareTrack(MKTrack *track) {
    if (Utils::isLocalFile(track->url())) {
        return share(Utils::unescape(track->url().toString()));
    }

    QVariantMap attributes;
    attributes["title"] = track->title();
    attributes["source"] = track->url();
    return share("text/url", track->url().toString(), attributes);
}
