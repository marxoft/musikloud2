...
MySheet {
    ...
    content: Item {
        ...
        Column {
            ...
            ValueSelector {
                ...
                selectedIndex: Math.max(0, model.match("name",Settings.defaultSearchType(Resources.SOUNDCLOUD)))
                onAccepted: Settings.setDefaultSearchType(Resources.SOUNDCLOUD, model.data(selectedIndex, "name"))
            }
        }
    ...
    onAccepted: {
        if (!mainPage.showResourceFromUrl(searchField.text)) {
            ...
            mainPage.search(Resources.SOUNDCLOUD, searchField.text, typeSelector.value.type, typeSelector.value.order);
        }
    }
