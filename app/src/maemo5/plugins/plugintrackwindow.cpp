...
void PluginTrackWindow::loadTrackUi() {
    ...
    if (ResourcesPlugins::instance()->resourceTypeIsSupported(m_track->service(), Resources::COMMENT)) {
...
void PluginTrackWindow::getRelatedTracks() {
    ...
    m_relatedModel->list(m_track->id());
}
...
void PluginTrackWindow::downloadTrack() {
    ...
    PluginDownloadDialog *dialog = new PluginDownloadDialog(m_track->service(), m_track->id(), m_track->streamUrl(),
                                                            m_track->title(), this);
    dialog->open();
}
...
void PluginTrackWindow::downloadRelatedTrack() {
    if ((!isBusy()) && (m_relatedView->currentIndex().isValid())) {
        QString id = m_relatedView->currentIndex().data(PluginTrackModel::IdRole).toString();
        QString title = m_relatedView->currentIndex().data(PluginTrackModel::TitleRole).toString();
        QUrl streamUrl = m_relatedView->currentIndex().data(PluginTrackModel::StreamUrlRole).toString();
        
        PluginDownloadDialog *dialog = new PluginDownloadDialog(m_track->service(), id, streamUrl, title, this);
        dialog->open();
    }
}
...
void PluginTrackWindow::showArtist(const QModelIndex &index) {
    ...
    PluginArtistWindow *window = new PluginArtistWindow(m_track->service(),
                                                    index.data(PluginCommentModel::ArtistIdRole).toString(), this);
    window->show();
}
...
void PluginTrackWindow::showComments() {
    if (!m_commentView) {
        ...
        m_commentModel->list(m_track->id());
    }
