...
#ifdef SYMBIAN_OS
    db.setDatabaseName("musikloud2.db");
#else
    QDir().mkpath(DATABASE_PATH);
    db.setDatabaseName(DATABASE_PATH + "musikloud2.db");
#endif
    if (!db.isOpen()) {
        db.open();
    }
    
    QSqlQuery query = db.exec("CREATE TABLE IF NOT EXISTS soundcloudAccounts (userId TEXT UNIQUE, username TEXT, \
